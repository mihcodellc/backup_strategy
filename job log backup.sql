...
--Create a job
IF NOT EXISTS(select '1' from msdb.dbo.sysjobs_view WHERE name='BackupThemUp_Log') BEGIN
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'BackupThemUp_Log', 
			@enabled=1, 
			...
			@description=N'Backup the selected databases every 10mn from 6am-6pm. ', 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
			...
			@command=N'exec master.dbo.[sp_DatabaseBackup] @Databases=''iThinkhealth'', @Directory=''C:\Backups\'',
						  @BackupType=''LOG'', @Verify=''Y'', @Compress=''Y'', @CheckSum=''Y'', @NoRecovery=''N'', @DirectoryStructure = NULL,
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, 
			@name=N'Schedule_BackupThemUp_Log', 
			@enabled=1, 
			@freq_type=4,    -- 1 Once 4 Daily 8 Weekly 16 Monthly 32 (monthly relative) 64 When Agent starts
			@freq_interval=1, 
			@freq_subday_type=4, -- 0x1 specified time  0x4 Minutes	 0x8 Hours
			@freq_subday_interval=10, --periods to occur between each execution
...
END
ELSE
		SELECT 'BackupThemUp_Log exists. Check if its config match your requirements ' PAY_ATTENTION

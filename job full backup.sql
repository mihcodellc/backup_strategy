...
--Create a job
IF NOT EXISTS(select '1' from msdb.dbo.sysjobs_view WHERE name='BackupThemUp_Full') BEGIN
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'BackupThemUp_Full', 
			@enabled=1, 
			...
			@description=N'Backup the selected databases at 7pm. ', 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
		...
		@command=N'exec master.dbo.[sp_DatabaseBackup] @Databases=''%Thinkhealth%, msdb'', @Directory=''C:\Backups\'',
						  @BackupType=''FULL'', @Verify=''Y'', @Compress=''Y'', @CheckSum=''Y'', @NoRecovery=''N'', @DirectoryStructure = NULL,
						  @FileName=''{DatabaseName}_{BackupType}_{Year}{Month}{Day}_{Hour}{Minute}{Second}_{FileNumber}.{FileExtension}'',
						  @FileExtensionFull=''bak'', @FileExtensionDiff=''DIF'', @FileExtensionLog=''trn'', @Init=''N'', @LogToTable=''Y'',
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, 
			@name=N'Schedule_BackupThemUp_Full', 
			@enabled=1, 
			@freq_type=4,    -- 1 Once 4 Daily 8 Weekly 16 Monthly 32 (monthly relative) 64 When Agent starts
			@freq_interval=1, 
			@freq_subday_type=1, -- 0x1 specified time  0x4 Minutes	 0x8 Hours
			@freq_subday_interval=0, --periods to occur between each execution
			@freq_relative_interval=0, 
			@active_start_date=20191101, 
			@active_end_date=99991231, 
			@active_start_time=190000, --HHMMSS
			@active_end_time=0 --, --HHMMSS
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
END
ELSE
		SELECT 'BackupThemUp_Full exists. Check if its config match your requirements ' PAY_ATTENTION
...
END
ELSE
	SELECT 'Alert_Severity16 exists under another name. Check if its config match your requirements ' AS PAY_ATTENTION
